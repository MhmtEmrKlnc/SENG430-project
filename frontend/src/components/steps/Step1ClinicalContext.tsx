    ...
    return (
      <div className="step-container">
        <div className="text-center py-16">
          <Stethoscope className="mx-auto h-16 w-16 text-brand-teal/40 mb-6" aria-hidden="true" />
          <h1 className="text-3xl font-bold text-brand-navy mb-3">
            {t('steps.1.startHeading')}
          </h1>
          {/* LCP element — rendered immediately, no lazy wrapper */}
          <p className="text-muted-foreground text-lg mb-2 max-w-xl mx-auto" style={{ contentVisibility: 'visible' }}>
            {t('steps.1.startDesc')}
          </p>
          ...
          <div className="mt-12 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 max-w-4xl mx-auto">
            {DOMAINS.map((d) => (
              <button
                ...
                aria-pressed={d.id === selectedDomainId}
                className={cn(
                  'text-left rounded-lg border bg-surface-card p-3 transition-colors',
                  'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-blue',
                  d.id === selectedDomainId
                    ? 'border-brand-teal ring-2 ring-brand-teal/40'
                    : 'border-border-subtle hover:border-brand-teal/40 hover:bg-brand-teal/5'
                )}
              >
