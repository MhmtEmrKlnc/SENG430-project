...
export default function ColumnMapperModal({ summary, onClose, onSave }) {
  ...
  const saveToBackend = async () => {
    try {
      setSaving(true);
      setBanner(null);
      const res = await api.post('/projects/1/column-mapper/save/', {
      ...
      const b = err.response?.data?.banner;
      setBanner(b || {
        type: 'error',
        title: 'Save failed',
        message: err.response?.data?.error || "Failed to save column mapping.",
      });
      return null;
    } finally {
      setSaving(false);
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '2rem' }}>
              <button
                ...
                disabled={saving}
              >
                {saving ? <Loader2 className="animate-spin" size={16} /> : 'Validate Schema'}
              </button>
              <button
